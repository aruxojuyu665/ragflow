...
import copy
from opensearchpy import OpenSearch as Elasticsearch, NotFoundError
...
class ESConnection(DocStoreConnection):
    def __init__(self):
        ...
        # Try to ping OpenSearch with retries
        ...
        for attempt in range(ATTEMPT_TIME):
            try:
                if self.es.ping():
                    ...
                    logger.warning(f"OpenSearch {settings.ES['hosts']} ping failed on attempt {attempt + 1}/{ATTEMPT_TIME}")
                    if attempt < ATTEMPT_TIME - 1:
                        time.sleep(2)
            except Exception as e:
                logger.warning(f"OpenSearch ping check failed on attempt {attempt + 1}/{ATTEMPT_TIME}: {e}")
                if attempt < ATTEMPT_TIME - 1:
                    time.sleep(2)
        
        if not ping_successful:
            logger.error(f"OpenSearch {settings.ES['hosts']} is not responding after {ATTEMPT_TIME} attempts")
            raise Exception(f"OpenSearch connection failed: {settings.ES['hosts']} is not responding")
        # Version check disabled for OpenSearch compatibility
    ...
    def _connect(self):
        self.es = Elasticsearch(
            ...
            http_auth=(settings.ES["username"], settings.ES[
    ...
    def search(
        ...
        for m in matchExprs:
            if isinstance(m, MatchTextExpr):
                ...
                # OpenSearch k-NN query with filter parameter
                # Build knn query with vector and k
                knn_field_query = {
                    "vector": list(m.embedding_data),
                    "k": m.topn
                }
                
                # Add min_score if similarity threshold is set
                if similarity > 0:
                    knn_field_query["min_score"] = similarity
                
                # Add filter inside knn query (OpenSearch syntax)
                if bqry and bqry.to_dict().get("bool", {}).get("filter"):
                    knn_field_query["filter"] = {
                        "bool": {
                            "filter": bqry.to_dict()["bool"]["filter"]
                        }
                    }
                
                # Build final knn query
                knn_query = {
                    "knn": {
                        m.vector_column_name: knn_field_query
                    }
                }
                
                # Apply knn query using update_from_dict
                s = s.update_from_dict({"query": knn_query})
    ...
    def insert(self, documents: list[dict], indexName: str, knowledgebaseId: str = None) -> list[str]:
        ...
        for _ in range(ATTEMPT_TIME):
            try:
                res = []
                r = self.es.bulk(index=indexName, body=operations,
